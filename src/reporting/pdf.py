...
def generate_pdf_report(scan_data: Dict[str, Any]) -> str:
    ...
    if vulnerable:
        ...
        for r in vulnerable:
            indicators_str = ", ".join(r.get("indicators", []))[:40]
            table_data.append([
                r.get("probe_type", ""),
                r.get("severity", "HIGH"),
        ...
        findings_table = Table(
            table_data,
            colWidths=[1.2*inch, 0.8*inch, 0.5*inch, 1.0*inch, 0.7*inch, 2.5*inch]
        )
